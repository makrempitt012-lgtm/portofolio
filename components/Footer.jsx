  ...
  return (
    <footer className="relative pt-16 pb-8 px-4 sm:px-6">
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            {socialLinks.map((social, index) => (
              <a
                ...
                className="w-10 h-10 rounded-xl bg-slate-900/60 border border-slate-800 flex items-center justify-center text-slate-300 hover:text-indigo-400 hover:border-indigo-500/30 hover:bg-indigo-500/5 transition-all duration-200"
              >
                <social.icon size={16} />
              </a>
            ))}
        ...
        {/* Bottom Bar */}
        <div className="flex flex-col items-center justify-center gap-4 pt-8 border-t border-slate-800/50">
          ...
          <button
            onClick={scrollToTop}
            className="flex items-center gap-2 text-sm text-slate-500 hover:text-white transition-colors duration-300 group"
          >
            Kembali ke Atas
            <ArrowUp
              size={14}
